...
names=[ra_name, dec_name, z_name, obs_name, 'DSt', 'DSx', 'W_l', 'radius']
...
for i, name_file in enumerate(file):
    ...
    data_to_save = [ra, dec, z, obs, data['gt'], data['gx'], data['W_l'], data['radius']]
    for s, n in enumerate(names): ind_profile[n].append(data_to_save[s])
    #if i > 1000: break

edit.save_pickle(Table(ind_profile), 'test_table_ind_profile')
