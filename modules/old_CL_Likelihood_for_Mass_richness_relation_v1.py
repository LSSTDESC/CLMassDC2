...
class WL_Mass_Richness():
    ...
    def lnLikelihood_individual_zrichness(self, thetaMC):
        ...
        for i, logm in enumerate(self.logm):
            logm_ind_mean = self.mu_logM_lambda(self.richness_individual[i], self.z_individual[i], thetaMC) 
            logm_ind = logm_ind_mean# + np.random.randn(len(logm_ind_mean))*(self.scatter_logm**2)**.5
            m_ind = 10**logm_ind
            m_G=(m_ind)**Gamma
