...
def _plot_contours(ax, pos2d, binsx = None, binsy = None,c=None):
    
    x, y = pos2d
    H, xedges, yedges = np.histogram2d(x, y, bins=[binsx, binsy], density = True)
    X, Y = np.meshgrid(np.mean(binning(binsx), axis = 1), np.mean(binning(binsy), axis = 1))
    ax.contour(X, Y, H, colors=c,);
...
def map(func, iter, ncores = None, ordered=True, progress=True):
...
def weightedcovar(x, y, w=None):
    ...
    #return np.cov(x, y, rowvar=1, bias=0, fweights=None, aweights=w)
